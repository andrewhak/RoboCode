...
def can_send_messages(bus, messages):
    expected_responses = {1, 2, 3, 4, 5, 6}  # IDs of motors expecting responses
    received_responses1 = set()
    received_responses2 = set()
    ...
    timeout = 5  # Increase the timeout value
    ...
    while True:
        ...
        if received_msg is not None:
            ...
            if (received_msg.arbitration_id in expected_responses) and (received_msg.data[1] == 1):
                received_responses1.add(received_msg.arbitration_id)
            ...
            if (received_msg.arbitration_id in expected_responses) and (received_msg.data[1] == 2) and (received_msg.arbitration_id in received_responses1):
                received_responses2.add(received_msg.arbitration_id)
        

        # Check if responses for all expected motors are received
        if received_responses1 == expected_responses:
            # Check if the status is 2 (run complete) for all received responses
            if set(received_responses2) == set(expected_responses):
